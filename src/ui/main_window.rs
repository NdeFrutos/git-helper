...
use crate::{
    actions::{
        ...
        FocusPreviousChange, GenerateCommitMessage, MoveRepositoryLeft, MoveRepositoryRight,
        NextRepository, OpenInEditor, OpenRepository, OpenTerminalHere, PreviousRepository,
        RefreshRepository, ReopenClosedRepository, RevealInFileManager, SelectAllChanges,
        ShowChanges, ShowHistory, ShowSummary, StageSelection, SummaryActivateRow, SummaryNextRow,
        SummaryPreviousRow, ToggleFavoriteRepository, ToggleFocusedChange, UnstageSelection,
    },
    app::AppStartup,
    ...
    domain::{
        ...
        CommitPreferenceField, CommitSummary, EffectiveCommitPreferences, ExternalCommand,
        ...
        MutationState, OperationKind, PreferenceScope, RefreshState, RemoteOperationPlan,
        RepositoryId, RepositorySession, RepositorySummaryRow, RepositoryView, SearchQuery,
        SnapshotPresentation, SshCloneMapping, SystemClock, TemplateApplication, ToolArgument,
        WorkingTreeSnapshot, append_repository_path, build_repository_summaries, change_matches,
        commit_matches, commit_message_guidance, contains_repository_path, cycle_commit_preference,
        effective_commit_preferences, forget_repository_path, format_change_counters,
        format_periodic_fetch_interval_label, format_sync_counters, moved_tab_index,
        next_periodic_fetch_interval, normalized_repo_key, periodic_fetch_poll_interval,
        plan_commit_template, primary_remote_label, promote_repository_path, select_periodic_fetch,
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                match result {
                    ...
                    Err(error) => {
                        this.report_known_repository_failure(
                            &requested_path,
                            &guided_error_text(&error),
                        );
                    }
                }
                cx.notify();
                ...
                div()
                    ...
                    .child(
                        action_button("open-terminal", "Terminal", true)
                            .aria_label("Abrir una terminal en la raíz del repositorio (Ctrl+Alt+T)")
                            .on_click(cx.listener(move |this, _, _, cx| {
                        ...
                        root.child(self.render_toolbar(&repository, cx))
                            ...
                            .child(match repository.selected_view {
                                RepositoryView::Changes => {
                                    self.render_changes(&repository, window, cx)
                                }
