...
use crate::domain::{
    AppSettings, AppState, ChangeCounters, ChangeSelectionState, HistorySnapshot,
    MAX_FAVORITE_REPOSITORIES, MAX_RECENT_REPOSITORIES, MAX_RECENTLY_CLOSED_REPOSITORIES,
    MutationState, RefreshCoordinator, RefreshState, RepositoryId, RepositorySession,
    RepositoryView, SshCloneMapping, WorkingTreeSnapshot, deduplicate_repository_paths,
};
...
/// v5 → v6: aparecen favoritos y pestañas cerradas; deja las tres listas de rutas sin
...
mod tests {
    ...
    #[test]
    fn migrates_v4_state_without_commit_message_preferences_or_favorites() {
        let temporary_directory = tempdir().expect("debe crear el temporal");
        let state_path = temporary_directory.path().join("state.json");
        fs::write(
            &state_path,
    ...
    fn migration_keeps_favorites_written_by_the_current_schema() {
        ...
        .expect("debe escribir el fixture v6");
        let store = AppStateStore::new(state_path);
